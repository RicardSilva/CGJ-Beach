...
matrix4d ViewMatrix11 = matrixFactory::createViewMatrix(*new vector3d(5,5,5),*new vector3d(0, 0, 0), *new vector3d(0, 1, 0));
// Eye(-5,-5,-5) Center(0,0,0) Up(0,1,0)
...
matrix4d ViewMatrix22 = matrixFactory::createViewMatrix(*new vector3d(-5, -5, -5), *new vector3d(0, 0, 0), *new vector3d(0, 1, 0));
...
matrix4d OrthographicMatrix1 = matrixFactory::createOrthographicMatrix(-2,2,-2,2,1,10);

// Perspective Fovy(30) Aspect(640/480) NearZ(1) FarZ(10)

matrix4d ProjectionMatrix2 = matrixFactory::createProjectionMatrix(30,640/480,1,10);

const Matrix ProjectionMatrix3 = {
...
{
	glBindBuffer(GL_UNIFORM_BUFFER, VboId[1]);
	glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(Matrix), ViewMatrix11.getM());
	//glBufferSubData(GL_UNIFORM_BUFFER, sizeof(Matrix), sizeof(Matrix), ProjectionMatrix1);
	glBufferSubData(GL_UNIFORM_BUFFER, sizeof(Matrix), sizeof(Matrix), ProjectionMatrix2.getM());
	glBindBuffer(GL_UNIFORM_BUFFER, 0);
...
void MouseMotion(int x, int y)
{
	...
	if (g_bButton1Down)
	{
		...
		//ProjectionMatrix2 = matrixFactory::createProjectionMatrix(g_fViewDistance, 640 / 480, 1, 10);
		std::cout << x<<" ";
		ViewMatrix11 = matrixFactory::createViewMatrix(*new vector3d(5+ (x - g_xClick)/3.0f, 5, 5), *new vector3d(0, 0, 0), *new vector3d(0, 1, 0));
		glutPostRedisplay();
	}
}
...
{
	std::cout << ViewMatrix11;
	std::cout << ProjectionMatrix2;
	setupGLUT(argc, argv);
	setupGLEW();
	setupOpenGL();
	createShaderProgram();
